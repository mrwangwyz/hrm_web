<template>
	<section>
		...
		<!--private CourseType parent;                              // 上级部门 部门对象-->
		...
		<el-dialog title="添加或修改" :visible.sync="formVisible" :close-on-click-modal="false">
			<el-form :model="courseType" label-width="80px" :rules="formRules" ref="courseType">
				...
				<el-form-item label="标识" prop="sn">
					<el-input v-model="courseType.sn" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="路径" prop="dirPath">
					<el-input v-model="courseType.dirPath" auto-complete="off"></el-input>
				</el-form-item>
				<el-form-item label="状态" prop="state">
					<el-radio-group v-model="courseType.state" >
						<el-radio checked="checked" :label="0">正常</el-radio>
						<el-radio  :label="-1">停用</el-radio>
					</el-radio-group>
				</el-form-item>
				<el-form-item label="部门经理" prop="manager">
					<el-select v-model="courseType.manager" placeholder="请选择">
						<el-option
	...
	export default {
		...
		methods: {
                ...
                this.$refs.courseType.validate((valid) => {
                    ...
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            //判断是否有id有就是修改,否则就是添加
							this.$http.put("/courseType",para).then((res) => {
								this.$message({
									message: '操作成功!',
									type: 'success'
								});
            ...
            getCourseTypes(){
				...
				/* 查询所有
				this.$http.patch("/courseType",para) //$.Post(.....)
					.then(result=>{
                        console.log(result);
			...
			del(row){
                ...
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    this.$http.delete("/courseType/"+id)
                        .then(result=>{
