<template>
	<section>
		...
		<!--private Course parent;                              // 上级部门 部门对象-->
			...
			<el-form :model="course" label-width="80px" :rules="formRules" ref="course">
				...
				<el-form-item label="课程等级" prop="grade">
						<el-radio-group v-model="course.grade">
							<el-radio v-for="courseLeavel in courseLeavels" :label="courseLeavel.id">{{courseLeavel.name}}</el-radio>
						</el-radio-group>
				</el-form-item>
        ...
        methods: {
            add() {
                ...
                this.course = {
                	...
                	this.getCourseLeaves();
            },
			getCourseLeaves(){
                this.$http.get("/sysmanage/systemdictionaryitem/listSn?sn=courseLeavel") //$.Post(.....)
                    .then(result => {
						this.courseLeavels=result.data
                    });
			},
			...
			getCourses(){
				...
				/* 查询所有
			this.$http.patch("/course",para) //$.Post(.....)
				.then(result=>{
					console.log(result);
